...
Auto-Publish's core (M2); real posts have been published by manual runs and by
scheduled run `36209353553` (2026-09-26, gate `pass`). Not built: the `SAFETY_PAUSE` trigger and proactive
...
|---|---|---|---|
| 1 | Publication Core Loop — LinkedIn and Habr both publish automatically, end to end, on real data | COMMITTED, in progress | Per `docs/PROJECT.md`'s Definition of Done. A publication registry is required before the first automatic run — see `docs/BACKLOG.md`'s `[B-064]`; its schema and writer are implemented. LinkedIn first, then Habr: LinkedIn publishing has run manually and via a scheduled run (`36209353553`, 2026-09-26, gate `pass`), no Habr publication component exists. |
